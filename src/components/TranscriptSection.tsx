...
export default function TranscriptSection({ transcriptData, formatTime, onTranscriptClick }: TranscriptSectionProps) {
  const blockBorder = useColorModeValue("rgba(220,220,230,0.5)", "rgba(60,60,80,0.3)");
  const fontSize = 19;
      ...
      >
        {Array.isArray(transcriptData?.segments) && transcriptData.segments.length > 0 ? (
          ...
          <Box
            ...
            gap={3}
            _hover={{
              background: useColorModeValue('rgba(245,245,255,0.45)', 'rgba(40,40,60,0.25)'),
              boxShadow: '0 2px 12px 0 rgba(128,90,213,0.08)',
              border: `1px solid ${useColorModeValue('#e2e8f0','#444b5a')}`
            }}
            style={{
          ...
          >
            ...
            <Box flex={1}>
              <Box fontSize="sm" color="#7c6ee6" fontWeight="bold" mb={1} textAlign="left">
                {seg.speaker ? seg.speaker : ""} <span style={{color:'#aaa', fontWeight:400, marginLeft:8}}>{formatTime(seg.start)} - {formatTime(seg.end)}</span>
              </Box>
              <Box fontSize="lg" fontFamily="inherit" whiteSpace="pre-line" textAlign="left" color="#222" style={{letterSpacing:0.1}}>{seg.text}</Box>
            </Box>
          </Box>
          ))
        ) : (
